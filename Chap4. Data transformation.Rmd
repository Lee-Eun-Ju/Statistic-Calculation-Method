...
cancel = flights %>% group_by(year,month, day) %>% 
  summarise(n = sum(is.na(dep_delay)|is.na(arr_delay)), 
            prop = mean(is.na(dep_delay)|is.na(arr_delay)),
            average_delay = mean(arr_delay>0, na.rm=TRUE))

cancel %>% ggplot(aes(n)) + geom_histogram()
cancel %>% ggplot(aes(x=prop, y=average_delay)) +
...
car =flights %>% group_by(carrier) %>% 
  summarise(n=sum(arr_delay>0, na.rm=TRUE)) %>% arrange(desc(n))

dis = flights %>% 
  group_by(carrier,dest) %>% 
  summarise(n=n(), delay_prop=mean(arr_delay>0, na.rm=TRUE))

ggplot(dis) +
  geom_bar(aes(x=carrier,y=delay_prop, colour=dest)) 

dis %>% 
  ggplot() +
  geom_point(aes(x=dest, y=prop, colour=carrier))
...
flights %>% group_by(carrier) %>% summarise(n)
            ...
            delay=sum(arr_delay>0, na.rm=TRUE),
            prop=mean(arr_delay>0, na.rm=TRUE))
car_dest %>% ggplot(mapping=aes(x=dest,delay)) + 
  geom_histogram() + facet_wrap(.~carrier)
  
ggplot(data=mpg) +
  geom_point(mapping=aes(x=displ,y=hwy)) +
  facet_wrap(.~cty)

  summarise(n=sum(arr_delay>0, na.rm=TRUE)) %>% arrange(desc(n)) %>% table() %>% t()
car_dest %>% ggplot(mapping=aes(x=carrier)) + geom_abline(mapping=aes(colour=dest)) 
```
...
#1-2
flights %>% group_by(tailnum) %>% 
  mutate(arr_delay_mean = mean(arr_delay, na.rm=TRUE)) %>% 
  select(tailnum, arr_delay_mean)
```
